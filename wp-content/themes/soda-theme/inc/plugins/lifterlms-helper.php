...
if ( ! class_exists( '\BuddyBossTheme\LifterLMSHelper' ) ) {
	...
	Class LifterLMSHelper {
		...
		public function __construct() {
			...
			add_action( 'lifterlms_single_course_after_summary', array( $this, 'lifterlms_template_single_reviews_course' ), 100 );
			...
			add_action( 'lifterlms_single_membership_before_summary', array( $this, 'bb_llms_single_membership_before_summary' ), 10 );

			remove_action( 'lifterlms_after_student_dashboard', 'lifterlms_template_student_dashboard_wrapper_close', 10 );
			add_action( 'lifterlms_after_student_dashboard', array( $this, 'bb_lifterlms_template_student_dashboard_wrapper_close' ), 12 );
			...
			add_action( 'wp_ajax_buddyboss_llms_get_course_participants',        array( $this, 'buddyboss_llms_get_course_participants' ) );
			add_action( 'wp_ajax_nopriv_buddyboss_llms_get_course_participants', array( $this, 'buddyboss_llms_get_course_participants' ) );
			...
			add_filter( 'llms_get_product_schedule_details', array( $this, 'buddyboss_llms_add_space_before_schedule_details' ), 9999, 2 );
		}
		...
		public function ajax_get_courses() {
			...
			if ( $c_q->have_posts() ) {
				...
				$view = isset( $_GET['view'] ) && in_array( $_GET['view'], array( 'list', 'grid' ) ) ? $_GET['view'] : 'grid'; // WPCS: loose comparison ok.
			...
			if ( get_post_meta( get_the_ID(), '_llms_reviews_enabled', true ) && is_user_logged_in() ) {
				...
				if ( get_post_meta( get_the_ID(), '_llms_multiple_reviews_disabled', true ) && $posts_array ) {
					...
					<div class="review_box" id="review_box">
						...
						<input style="margin:10px 0px" type="text" name="review_title" placeholder="<?php _e( 'Review Title', 'buddyboss-theme' ); ?>" id="review_title">
						<h5 style="color:red; display:none" id="review_title_error">
						<?php _e( 'Review Title is required.', 'buddyboss-theme' ); ?>
						</h5>
						<textarea name="review_text" placeholder="<?php _e( 'Review Text', 'buddyboss-theme' ); ?>" id="review_text"></textarea>
						...
						<span class="review_leave">
					   	<input type="submit" class="button" value="<?php _e( 'Leave Review', 'buddyboss-theme' ); ?>" id="llms_review_submit_button">
					</span>
						<!--</form>	-->
					</div>
